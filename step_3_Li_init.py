...
def main_3(filePath='ma_class/L%s.txt'):
    for j in range(16):
        f = np.loadtxt(filePath % j)
        f_1_10 = f[:, 1:11]
        ...
        #print(f_init.shape)
        np.savetxt('Li_init/L%s.txt' % j, f_init)#11列，不含预测类和分类列
    print('step_3:均线分类数据归一化单位化')
    print('每行除行平均，每列减去列最小值，除(列最大值-列最小值);使数据位于[0,1]')
    print("只取前11列数据，不包含预测列、分类序号列")
    print('数据保存于：Li_init/L%s.txt')
    print('------')

    return 0
if __name__ == '__main__':
    filePath = 'ma_class/L%s.txt'
