...
def main_2(filePath = 'raw_data_procession/gold_intex_-10_0.txt', ma_1 = 50, ma_2 = 90, k=100):
    ...
    for i in range(16):
        ...
        #print('L%s.shape:' % i, Ld.shape)
    print('step_2:用两条移动平均线的2阶差分将数据分为16类')
    print('去掉前k个数据：', k)
    print('增加一列：分类序号列')
    print('处理后数据总表保存在：ma_class/label_10_index.txt')
    print('分类后各类保存在：ma_class/L%s.txt')
    print('------')

if __name__=="__main__":
    filePath = 'raw_data_procession/gold_intex_-10_0.txt'
    ma_1 = 50
    ma_2 = 90
    k = 100     #去掉前100个数据
    main_2(filePath='raw_data_procession/gold_intex_-10_0.txt', ma_1=50, ma_2=90, k=100)
